...
namespace Exine.ExineObjects
{
    class MonsterObject : MapObject
    {
        ...
        {
            get { return AI == 64 || (AI == 81 && Direction == (ExineDirection)6) ? false : !Dead; }
        }
        ...
        public void Load(S.ObjectMonster info, bool update = false)
        {
            ...
            if (!update) ExineMainScene.Scene.MapControl.AddObject(this);
           ...
           //Console.WriteLine("Monster Name:" + Name + " BaseImage:" + BaseImage + " (short)BaseImage:" + (short)BaseImage + "ColorR:"+TintColor.R+" ColorG:"+ TintColor.G+" ColorB:"+ TintColor.B);
            ...
            if (Skeleton)
                ActionFeed.Add(new QueuedAction { Action = ExAction.Skeleton, Direction = Direction, Location = CurrentLocation });
            else if (Dead)
            ...
            switch (BaseImage)
            {
                ...
                case Monster.PowerUpBead:
                    if (!info.Extra) ActionFeed.Add(new QueuedAction { Action = ExAction.Appear, Direction = Direction, Location = CurrentLocation });
                    break;
            ...
            else if (CurrentAction == ExAction.SitDown)
            {
                PlayAppearSound();
            }
        ...
        public override bool ShouldDrawHealth()
        {
            ...
            //return Name.EndsWith(string.Format("({0})", User.Name)) || ExineScenes.Dialogs.GroupDialog.GroupList.Contains(name);
            return Name.EndsWith(string.Format("({0})", User.Name)) || ExineScenes.ExDialogs.GroupDialog.GroupList.Contains(name);
        }
        ...
        public override void Process()
        {
            ...
            switch (CurrentAction)
            {
                ...
                case ExAction.DashAttack:
                    ...
                    int i = CurrentAction == ExAction.Running ? 2 : 1;
        ...
        public bool SetAction()
        {
            ...
            else
            {
                ...
                switch (CurrentAction)
                {
                    ...
                    case ExAction.Pushed:
                        int i = CurrentAction == ExAction.Running ? 2 : 1;
                        temp = Functions.PointMove(CurrentLocation, Direction, CurrentAction == ExAction.Pushed ? 0 : -i);
                        break;
                    case ExAction.Jump:
                    case ExAction.DashAttack:
                ...
                switch (CurrentAction)
                {
                    ...
                    case ExAction.AttackRange3:
                        ...
                        break;
                    case ExAction.Special:
                        if (!Frames.TryGetValue(CurrentAction, out Frame))
                            Frames.TryGetValue(ExAction.Attack1, out Frame);
                        break;
                    case ExAction.Skeleton:
                        if (!Frames.TryGetValue(CurrentAction, out Frame))
                            Frames.TryGetValue(ExAction.Dead, out Frame);
                        break;
                    case ExAction.Hide:
                        switch (BaseImage)
                        {
                ...
                switch (CurrentAction)
                {
                    ...
                    case ExAction.Struck:
                        ...
                        for (int i = 0; i < MapControl.Objects.Count; i++)
                        {
                            ...
                            if (player.Class != ExineClass.Assassin || StruckWeapon == -1) break; //Archer?
                            StruckWeapon = 1;
                            break;
                        }
                        PlayFlinchSound();
        ...
        private void ProcessFrames()
        {
            ...
            if ((CurrentAction == ExAction.Standing || CurrentAction == ExAction.SitDown) && NextAction != null)
                SetAction();
            else if (CurrentAction == ExAction.Dead && NextAction != null && (NextAction.Action == ExAction.Skeleton || NextAction.Action == ExAction.Revive))
                SetAction();
        }
        ...
        public override void DrawEffects(bool effectsEnabled)
        {
            ...
            switch (BaseImage)
            {
                ...
                case Monster.Behemoth:
                    ...
                    if (CurrentAction != ExAction.Dead)
