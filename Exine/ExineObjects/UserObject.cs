...
namespace Exine.ExineObjects
{
    public class UserObject : PlayerObject
    {
        ...
        public virtual BuffDialog GetBuffDialog => ExineMainScene.Scene.BuffsDialog;
        ...
        public ExineDirection NextMagicDirection;
        ...
        private void RefreshEquipmentStats()
        {
            ...
            for (int i = 0; i < Equipment.Length; i++)
            {
                ...
                ItemInfo realItem = Functions.GetRealItem(temp.Info, Level, Class, ExineMainScene.ItemInfoList);
        ...
        {
            if (User != this) return;
            if (ExineMainScene.Scene.GuildDialog == null) return;
            for (int i = 0; i < ExineMainScene.Scene.GuildDialog.EnabledBuffs.Count; i++)
            {
                GuildBuff buff = ExineMainScene.Scene.GuildDialog.EnabledBuffs[i];
                if (buff == null) continue;
                if (!buff.Active) continue;

                if (buff.Info == null)
                {
                    buff.Info = ExineMainScene.Scene.GuildDialog.FindGuildBuffInfo(buff.Id);
                }
        ...
        public void BindAllItems()
        {
            for (int i = 0; i < Inventory.Length; i++)
            {
                ...
                ExineMainScene.Bind(Inventory[i]);
            }

            for (int i = 0; i < Equipment.Length; i++)
            {
                if (Equipment[i] == null) continue;
                ExineMainScene.Bind(Equipment[i]);
            }

            for (int i = 0; i < QuestInventory.Length; i++)
            {
                if (QuestInventory[i] == null) continue;
                ExineMainScene.Bind(QuestInventory[i]);
            }
        }
        ...
        public void GetMaxGain(UserItem item)
        {
            ...
            if (item.Info.Type == ItemType.Amulet)
            {
                ...
                {
                    ExineMainScene.Scene.ExChatDialog.ReceiveChat(GameLanguage.NoBagSpace, ChatType.System);
        ...
        {
            if (QueuedAction != null && !ExineMainScene.Observing)
            {
                if ((ActionFeed.Count == 0) || (ActionFeed.Count == 1 && NextAction.Action == ExAction.Stance))
                {
                    ActionFeed.Clear();
