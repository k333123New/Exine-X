...
{
    public class MailListDialog : ExineImageControl
    {
        ExineImageControl TitleLabel;
        MirButton HelpButton, CloseButton;
        ExineLabel TitleTypeLabel, TitleSenderLabel, TitleMessageLabel;
        //Send / Reply (Can only reply if index exists, button will disapear if not) / Read / Delete / Block List / Bug Report (new system??)

        ExineLabel PageLabel;
        ...
        public MailListDialog()
        {
            ...
            TitleSenderLabel = new ExineLabel
            {
                Text = "SENDER",
                Parent = this,
                Font = new Font(Settings.FontName, Settings.FontSize - 1, FontStyle.Italic),
            ...
            TitleMessageLabel = new ExineLabel
            {
                Text = "MESSAGE",
                Parent = this,
                Font = new Font(Settings.FontName, Settings.FontSize - 1, FontStyle.Italic),
            ...
            HelpButton.Click += (o, e) => ExineMainScene.Scene.HelpDialog.DisplayPage("");
            ...
            SendButton.Click += (o, e) =>
                {
                    ...
                    inputBox.OKButton.Click += (o1, e1) =>
                    {
                        ...
                        ExineMainScene.Scene.MailComposeLetterDialog.ComposeMail(inputBox.InputTextBox.Text);
            ...
            ReplyButton.Click += (o, e) =>
            {
                ...
                ExineMainScene.Scene.MailComposeLetterDialog.ComposeMail(SelectedMail.SenderName);
            };
            ...
            ReadButton.Click += (o, e) =>
            {
                ...
                {
                    ExineMainScene.Scene.MailReadParcelDialog.ReadMail(SelectedMail);
                }
                else
                {
                    ExineMainScene.Scene.MailReadLetterDialog.ReadMail(SelectedMail);
                }
            };
            ...
            DeleteButton.Click += (o, e) =>
            {
                ...
                {
                    ExineMessageBox messageBox = new ExineMessageBox("이 소포에는 아이템 또는 데니가 들어 있습니다. 삭제하시겠습니까?", MirMessageBoxButtons.YesNo);
        ...
        public void UpdateInterface()
        {
            ...
            PageCount = (int)Math.Ceiling((double)ExineMainScene.User.Mail.Count / 10);
            ...
            for (int i = 0; i < Rows.Length; i++)
            {
                if (i + StartIndex >= ExineMainScene.User.Mail.Count) break;
                ...
                {
                    Mail = ExineMainScene.User.Mail[i + StartIndex],
    ...
    public class MailItemRow : ExineControl
    {
        public ClientMail Mail = null;

        public ExineLabel SenderLabel, MessageLabel;
        public ExineImageControl IconImage, UnreadImage, ParcelImage, LockedImage, SelectedImage;
        ...
        public MailItemRow()
        {
            ...
            UnreadImage = new ExineImageControl
            {
                Index = 550,
                Library = Libraries.Prguse,
            ...
            LockedImage = new ExineImageControl
            {
                Index = 551,
                Library = Libraries.Prguse,
                Location = new Point(5, 17),
            ...
            ParcelImage = new ExineImageControl
            {
                Index = 552,
                Library = Libraries.Prguse,
        ...
        {
            base.OnMouseEnter();
            ExineMainScene.Scene.CreateMailLabel(Mail);          
        }
        protected override void OnMouseLeave()
        {
            base.OnMouseLeave();
            ExineMainScene.Scene.DisposeMailLabel();
        }
    }

    public class MailComposeLetterDialog : ExineImageControl
    {
        ExineLabel RecipientNameLabel;
        ExineTextBox MessageTextBox;
        ...
        public MailComposeLetterDialog()
        {
            ...
            RecipientNameLabel = new ExineLabel
            {
                Text = "",
                Parent = this,
            ...
            MessageTextBox = new ExineTextBox
            {
                ForeColour = Color.White,
                Parent = this,
                Font = new Font(Settings.FontName, 8F),
    ...
    public class MailComposeParcelDialog : ExineImageControl
    {
        public ExineLabel RecipientNameLabel, ParcelCostLabel, GoldSendLabel;
        ExineTextBox MessageTextBox;
        MirButton StampButton, SendButton, CancelButton, CloseButton;
        ExineImageControl ItemCover;
        ...
        public MailComposeParcelDialog()
        {
            ...
            ItemCover = new ExineImageControl
            {
                Index = 676,
            ...
            ParcelCostLabel = new ExineLabel
            {
                DrawFormat = TextFormatFlags.VerticalCenter,
                Font = new Font(Settings.FontName, 8F),
            ...
            GoldSendLabel.Click += (o, e) =>
            {
                ...
                {
                    MirAmountBox amountBox = new MirAmountBox("Send Amount:", 116, ExineMainScene.Gold);
                    ...
                    amountBox.OKButton.Click += (c, a) =>
                    {
                        if (amountBox.Amount > 0)
                        {
                            ...
                            ExineMainScene.Gold -= amountBox.Amount;
                        }
        ...
        public void Reset()
        {
            ExineMainScene.Gold += GiftGoldAmount;
            GiftGoldAmount = 0;
            Stamped = false;
        ...
        private void StampParcel()
        {
            if(!Stamped)
            {
                ...
                {
                    UserItem item = ExineMainScene.User.Inventory[i];
    ...
    public class MailReadLetterDialog : ExineImageControl
    {
        ExineLabel SenderNameLabel, DateSentLabel, MessageLabel;
        ...
        public MailReadLetterDialog()
        {
            ...
            SenderNameLabel = new ExineLabel
            {
                Parent = this,
                Font = new Font(Settings.FontName, 8F),
                ForeColour = Color.White,
    ...
    public class MailReadParcelDialog : ExineImageControl
    {
        ExineLabel SenderNameLabel, DateSentLabel, MessageLabel, GoldSendLabel;
