...
namespace Exine.MirScenesOld
{
    public sealed class GameSceneOld : ExineScene
    {
        public static GameSceneOld Scene;
        ...
        private void UpdateMouseCursor()
        {
            ...
            if (GameSceneOld.HoverItem != null)
            {
                if (GameSceneOld.SelectedCell != null && GameSceneOld.SelectedCell.Item != null && GameSceneOld.SelectedCell.Item.Info.Type == ItemType.Gem && CMain.Ctrl)
                {
        ...
        {
            if (GameSceneOld.Scene.KeyboardLayoutDialog.WaitingForBind != null)
            {
                GameSceneOld.Scene.KeyboardLayoutDialog.CheckNewInput(e);
                return;
            }
            ...
            foreach (KeyBind KeyCheck in CMain.InputKeys.Keylist)
            {
                ...
                switch (KeyCheck.function)
                {
                    ...
                    case KeybindOptions.Mount:
                        if (GameSceneOld.Scene.MountDialog.CanRide())
                            GameSceneOld.Scene.MountDialog.Ride();
                        break;
                    ...
                    case KeybindOptions.Closeall:
                        ...
                        GameSceneOld.Scene.DisposeItemLabel();
                        break;
                    ...
                    case KeybindOptions.AddGroupMember:
                        ...
                        GameSceneOld.Scene.GroupDialog.AddMember(MapObject.MouseObject.Name);
                        break;
                }
            }
        }
        ...
        public void ChangeSkillMode(bool? ctrl)
        {
            if (Settings.SkillMode || ctrl == true)
            {
                ...
                GameSceneOld.Scene.ChatDialog.ReceiveChat("[SkillMode Ctrl]", ChatType.Hint);
                GameSceneOld.Scene.OptionDialog.ToggleSkillButtons(true);
            }
            else if (!Settings.SkillMode || ctrl == false)
            {
                Settings.SkillMode = true;
                GameSceneOld.Scene.ChatDialog.ReceiveChat("[SkillMode ~]", ChatType.Hint);
                GameSceneOld.Scene.OptionDialog.ToggleSkillButtons(false);
            }
        }
        ...
        private void UserInformation(S.UserInformation p)
        {
            ...
            User.Load(p);
            MainDialog.PModeLabel.Visible = User.Class == ExineClass.Wizard || User.Class == ExineClass.Taoist;
            HasHero = p.HasHero;
        ...
        private void SplitItem(S.SplitItem p)
        {
            ...
            if (p.Grid == MirGridType.Inventory && (p.Item.Info.Type == ItemType.Potion || p.Item.Info.Type == ItemType.Scroll || p.Item.Info.Type == ItemType.Amulet || (p.Item.Info.Type == ItemType.Script && p.Item.Info.Effect == 1)))
            {
                ...
                {
                    for (int i = 4; i < GameSceneOld.User.BeltIdx; i++)
                    {
                        if (array[i] != null) continue;
                        array[i] = p.Item;
                        User.RefreshStats();
            ...
            for (int i = GameSceneOld.User.BeltIdx; i < array.Length; i++)
            {
                if (array[i] != null) continue;
                array[i] = p.Item;
                User.RefreshStats();
            ...
            for (int i = 0; i < GameSceneOld.User.BeltIdx; i++)
            {
                if (array[i] != null) continue;
                array[i] = p.Item;
                User.RefreshStats();
        ...
        private void MountUpdate(S.MountUpdate p)
        {
            ...
            GameSceneOld.Scene.MountDialog.RefreshDialog();
            GameSceneOld.Scene.Redraw();
        }
        ...
        private void FishingUpdate(S.FishingUpdate p)
        {
            ...
            GameSceneOld.Scene.FishingStatusDialog.ProgressPercent = p.ProgressPercent;
            GameSceneOld.Scene.FishingStatusDialog.ChancePercent = p.ChancePercent;

            GameSceneOld.Scene.FishingStatusDialog.ChanceLabel.Text = string.Format("{0}%", GameSceneOld.Scene.FishingStatusDialog.ChancePercent);

            if (p.Fishing)
                GameSceneOld.Scene.FishingStatusDialog.Show();
            else
                GameSceneOld.Scene.FishingStatusDialog.Hide();
        ...
        {
            ClientQuestInfo quest = GameSceneOld.QuestInfoList.FirstOrDefault(e => e.Index == p.QuestIndex);
        ...
        private void ChangeQuest(S.ChangeQuest p)
        {
            switch(p.QuestState)
            {
                case QuestState.Add:
                    ...
                    {
                        GameSceneOld.Scene.QuestTrackingDialog.AddQuest(p.Quest, true);
                    }

                    if (p.TrackQuest)
                    {
                        GameSceneOld.Scene.QuestTrackingDialog.AddQuest(p.Quest);
                    }
                ...
                case QuestState.Remove:
                    ...
                    GameSceneOld.Scene.QuestTrackingDialog.RemoveQuest(p.Quest);

                    break;
            }

            GameSceneOld.Scene.QuestTrackingDialog.DisplayQuests();
        ...
        {
            GameSceneOld.NPCID = p.NPCID; //Updates the client with the correct NPC ID if it's manually called from the client
        }
        ...
        {
            GameSceneOld.DefaultNPCID = p.ObjectID; //Updates the client with the correct Default NPC ID
        }
        ...
        {            
            NewHeroDialog.WarriorButton.Visible = p.CanCreateClass[(int)ExineClass.Warrior];
            NewHeroDialog.WizardButton.Visible = p.CanCreateClass[(int)ExineClass.Wizard];
            NewHeroDialog.TaoistButton.Visible = p.CanCreateClass[(int)ExineClass.Taoist];
            NewHeroDialog.AssassinButton.Visible = p.CanCreateClass[(int)ExineClass.Assassin];
            NewHeroDialog.ArcherButton.Visible = p.CanCreateClass[(int)ExineClass.Archer];
        ...
        {
            MirMessageBox messageBox = new MirMessageBox(string.Format("{0} (Level {1}) has requested you teach him the ways of the {2}.", p.Name, p.Level, GameSceneOld.User.Class.ToString()), MirMessageBoxButtons.YesNo);
        ...
        private void ReceiveMail(S.ReceiveMail p)
        {
            ...
            GameSceneOld.Scene.MailListDialog.UpdateInterface();
        }
        ...
        private void MailSendRequest(S.MailSendRequest p)
        {
            ...
            inputBox.OKButton.Click += (o1, e1) =>
            {
                GameSceneOld.Scene.MailComposeParcelDialog.ComposeMail(inputBox.InputTextBox.Text);
                GameSceneOld.Scene.InventoryDialog.Show();
        ...
        private void MailSent(S.MailSent p)
        {
            ...
            GameSceneOld.Scene.MailComposeParcelDialog.Hide();
        }
        ...
        private void ParcelCollected(S.ParcelCollected p)
        {
            switch(p.Result)
            {
                ...
                case 0:
                    ...
                    break;
                case 1:
                    GameSceneOld.Scene.MailReadParcelDialog.Hide();
                    break;
            }
        }
        ...
        {
            if(GameSceneOld.Scene.MailComposeParcelDialog.Visible)
            {
                if (p.Cost > 0)
                    SoundManager.PlaySound(SoundList.Gold);

                GameSceneOld.Scene.MailComposeParcelDialog.ParcelCostLabel.Text = p.Cost.ToString();
            }
        }
        ...
        private void NewIntelligentCreature(S.NewIntelligentCreature p)
        {
            ...
            inputBox.InputTextBox.Text = GameSceneOld.User.IntelligentCreatures[User.IntelligentCreatures.Count-1].CustomName;
            inputBox.OKButton.Click += (o1, e1) =>
            {
                if (IntelligentCreatureDialog.Visible) IntelligentCreatureDialog.Update();//refresh changes
                GameSceneOld.User.IntelligentCreatures[User.IntelligentCreatures.Count - 1].CustomName = inputBox.InputTextBox.Text;
                Network.Enqueue(new C.UpdateIntelligentCreature { Creature = GameSceneOld.User.IntelligentCreatures[User.IntelligentCreatures.Count - 1] });
                inputBox.Dispose();
            };
            inputBox.Show();
        }
        ...
        {
            GameSceneOld.Scene.FriendDialog.Friends = p.Friends;

            if (GameSceneOld.Scene.FriendDialog.Visible)
            {
                GameSceneOld.Scene.FriendDialog.Update(false);
            }
        }

        private void LoverUpdate(S.LoverUpdate p)
        {
            GameSceneOld.Scene.RelationshipDialog.LoverName = p.Name;
            GameSceneOld.Scene.RelationshipDialog.Date = p.Date;
            GameSceneOld.Scene.RelationshipDialog.MapName = p.MapName;
            GameSceneOld.Scene.RelationshipDialog.MarriedDays = p.MarriedDays;
            GameSceneOld.Scene.RelationshipDialog.UpdateInterface();
        }

        private void MentorUpdate(S.MentorUpdate p)
        {
            GameSceneOld.Scene.MentorDialog.MentorName = p.Name;
            GameSceneOld.Scene.MentorDialog.MentorLevel = p.Level;
            GameSceneOld.Scene.MentorDialog.MentorOnline = p.Online;
            GameSceneOld.Scene.MentorDialog.MenteeEXP = p.MenteeEXP;

            GameSceneOld.Scene.MentorDialog.UpdateInterface();
        }
        ...
        {
            GameSceneOld.Scene.TimerControl.AddTimer(p);
        }

        private void ExpireTimer(S.ExpireTimer p)
        {
            GameSceneOld.Scene.TimerControl.ExpireTimer(p.Key);
        }

        private void SetCompass(S.SetCompass p)
        {
            GameSceneOld.Scene.CompassControl.SetPoint(p.Location);
        }

        private void Roll(S.Roll p)
        {
            GameSceneOld.Scene.RollControl.Setup(p.Type, p.Page, p.Result, p.AutoRoll);
        }
    ...
    public sealed class MapControl : ExineControl
    {
        ...
        {
            get { return GameSceneOld.User == null ? Point.Empty : new Point(MouseLocation.X / CellWidth - OffSetX, MouseLocation.Y / CellHeight - OffSetY).Add(GameSceneOld.User.CurrentLocation); }
        }
        ...
        public static long NextAction
        {
            ...
            set
            {
                if (GameSceneOld.Observing) return;
                nextAction = value;
            }
        }
        ...
        public static bool AutoRun
        {
            ...
            set
            {
                ...
                _autoRun = value;
                if (GameSceneOld.Scene != null)
                    GameSceneOld.Scene.ChatDialog.ReceiveChat(value ? "[AutoRun: On]" : "[AutoRun: Off]", ChatType.Hint);
            }
        ...
        {
            GameSceneOld.Scene.NPCDialog.Hide();
        ...
        public void LoadMap()
        {
            ...
            //PathFinder = new PathFinder(this); //231106 remove
                ...
                for (int x = MapLocation.X + 2; x >= MapLocation.X - 2; x--)
                {
                    ...
                    for (int i = cell.CellObjects.Count - 1; i >= 0; i--)
                    {
                        ...
                        if (MapObject.MouseObject != ob)
                        {
                            if (ob.Dead)
                            {
                                if (!Settings.TargetDead && GameSceneOld.TargetDeadTime <= CMain.Time) continue;
        ...
        protected override void CreateTexture()
        {
            ...
            LightSetting setting = Lights == LightSetting.Normal ? GameSceneOld.Scene.Lights : Lights;

            if (setting != LightSetting.Day || GameSceneOld.User.Poison.HasFlag(PoisonType.Blindness))
            {
                DrawLights(setting);
            }

            if (Settings.DropView || GameSceneOld.DropViewTime > CMain.Time)
            {
                for (int i = 0; i < Objects.Count; i++)
                {
        ...
        private static void OnMouseClick(object sender, EventArgs e)
        {
            ...
            switch (me.Button)
            {
                case MouseButtons.Left:
                    {
                        AutoRun = false;
                        GameSceneOld.Scene.MapControl.AutoPath = false;
                        if (MapObject.MouseObject == null) return;
                        NPCObject npc = MapObject.MouseObject as NPCObject;
                        if (npc != null)
                        {
                            if (npc.ObjectID == GameSceneOld.NPCID && 
                                (CMain.Time <= GameSceneOld.NPCTime || GameSceneOld.Scene.NPCDialog.Visible))
                            {
                                return;
                            }
                            ...
                            GameSceneOld.NPCTime = CMain.Time + 5000;
                            GameSceneOld.NPCID = npc.ObjectID;
                ...
                case MouseButtons.Right:
                    {
                        ...
                        if (MapObject.MouseObject == null)
                        {
                            if (Settings.NewMove && MapLocation != MapObject.User.CurrentLocation && GameSceneOld.Scene.MapControl.EmptyCell(MapLocation))
                            {
                                var path = GameSceneOld.Scene.MapControl.PathFinder.FindPath(MapObject.User.CurrentLocation, MapLocation, 20);

                                if (path != null && path.Count > 0)
                                {
                                    GameSceneOld.Scene.MapControl.CurrentPath = path;
                                    GameSceneOld.Scene.MapControl.AutoPath = true;
                        ...
                        if (CMain.Ctrl)
                        {
                            ...
                            if (hero != null &&
                                ...
                                CMain.Time >= GameSceneOld.InspectTime)
                            {
                                GameSceneOld.InspectTime = CMain.Time + 500;
        ...
        private static void OnMouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (GameSceneOld.SelectedCell != null)
            {
                ...
                MirItemCell cell = GameSceneOld.SelectedCell;
                if (cell.Item.Info.Bind.HasFlag(BindMode.DontDrop))
                {
                    ...
                    messageBox.Show();
                    GameSceneOld.SelectedCell = null;
                    return;
                }
            ...
            if (GameSceneOld.PickedUpGold)
            {
                MirAmountBox amountBox = new MirAmountBox(GameLanguage.DropAmount, 116, GameSceneOld.Gold);
                ...
                amountBox.Show();
                GameSceneOld.PickedUpGold = false;
            }
        ...
        private void CheckInput()
        {
            ...
            if (MapObject.TargetObject != null && !MapObject.TargetObject.Dead)
            {
                ...
                {
                    GameSceneOld.LogTime = CMain.Time + Globals.LogDelay;
                    ...
                    if (User.Class == ExineClass.Archer && User.HasClassWeapon && !User.RidingMount && !User.Fishing)//ArcherTest - non aggressive targets (player / pets)
                    {
                        ...
                        {
                            if (CMain.Time > GameSceneOld.AttackTime)
                            {
                        ...
                        else
                        {
                            ...
                            {
                                OutputDelay = CMain.Time + 1000;
                                GameSceneOld.Scene.OutputMessage("Target is too far.");
                            }
                        }
                        //  return;
                    }
                    ...
                    {
                        if (CMain.Time > GameSceneOld.AttackTime && CanRideAttack() && !User.Poison.HasFlag(PoisonType.Dazed))
                        {
            ...
            if (MouseControl == this)
            {
                ...
                if (AutoRun)
                {
                    if (GameSceneOld.CanRun && CanRun(direction) && CMain.Time > GameSceneOld.NextRunTime && User.HP >= 10 && (!User.Sneaking || (User.Sneaking && User.Sprint))) //slow remove
                    {
                ...
                switch (MapButtons)
                {
                    case MouseButtons.Left:
                        ...
                        if (CMain.Shift)
                        {
                            if (CMain.Time > GameSceneOld.AttackTime && CanRideAttack()) //ArcherTest - shift click
                            {
                                ...
                                if (GameSceneOld.User.DoubleSlash && (!User.HasClassWeapon && User.Weapon > -1)) return;
                        ...
                        {
                            if (CMain.Time > GameSceneOld.PickUpTime)
                            {
                                GameSceneOld.PickUpTime = CMain.Time + 200;
                                Network.Enqueue(new C.PickUp());
                            }
                            return;
                        }
                    ...
                    case MouseButtons.Right:
                        ...
                        GameSceneOld.CanRun = User.FastRun ? true : GameSceneOld.CanRun;

                        if (GameSceneOld.CanRun && CanRun(direction) && CMain.Time > GameSceneOld.NextRunTime && User.HP >= 10 && (!User.Sneaking || (User.Sneaking && User.Sprint))) //slow removed
                        {
                            int distance = User.RidingMount || User.Sprint && !User.Sneaking ? 3 : 2;
                            bool fail = false;
            ...
            if (AutoPath)
            {
                ...
                var path = GameSceneOld.Scene.MapControl.PathFinder.FindPath(MapObject.User.CurrentLocation, CurrentPath.Last().Location);
                ...
                if (CurrentPath.Count > 0)
                {
                    ...
                    if (GameSceneOld.CanRun && CanRun(dir) && CMain.Time > GameSceneOld.NextRunTime && User.HP >= 10 && CurrentPath.Count > (User.RidingMount ? 2 : 1))
                    {
        ...
        {
            if (CMain.Time < GameSceneOld.SpellTime || actor.Poison.HasFlag(PoisonType.Stun))
            {
                actor.ClearMagic();
                return;
            }
            ...
            if ((CMain.Time <= magic.CastTime + magic.Delay))
            {
                ...
                {
                    OutputDelay = CMain.Time + 1000;
                    GameSceneOld.Scene.OutputMessage(string.Format("You cannot cast {0} for another {1} seconds.", magic.Spell.ToString(), ((magic.CastTime + magic.Delay) - CMain.Time - 1) / 1000 + 1));
                }
            ...
            if (cost > actor.MP)
            {
                ...
                {
                    OutputDelay = CMain.Time + 1000;
                    GameSceneOld.Scene.OutputMessage(GameLanguage.LowMana);
                }
                actor.ClearMagic();
                return;
            }
            ...
            switch (magic.Spell)
            {
                ...
                case Spell.SummonSnakes:
                    ...
                    {
                        GameSceneOld.Scene.OutputMessage("You must be wearing a bow to perform this skill.");
                        actor.ClearMagic();
                        return;
                    }
                    if (actor.NextMagicObject != null)
        ...
        {
            if (GameSceneOld.User.RidingMount)
            {
                UserItem item = GameSceneOld.User.Equipment[(int)EquipmentSlot.Mount];
        ...
        {
            if (!GameSceneOld.User.HasFishingRod || GameSceneOld.User.FishingTime + 1000 > CMain.Time) return false;
            if (GameSceneOld.User.CurrentAction != ExAction.Standing) return false;
            if (GameSceneOld.User.Direction != dir) return false;
            if (GameSceneOld.User.TransformType >= 6 && GameSceneOld.User.TransformType <= 9) return false;
        ...
        public bool CanFly(Point target)
        {
            ...
            while (location != target)
            {
                ...
                if (location.X < 0 || location.Y < 0 || location.X >= GameSceneOld.Scene.MapControl.Width || location.Y >= GameSceneOld.Scene.MapControl.Height) return false;

                if (!GameSceneOld.Scene.MapControl.ValidPoint(location)) return false;
            }
