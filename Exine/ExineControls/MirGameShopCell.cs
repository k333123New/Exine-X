...
{
    public sealed class GameShopCell : ExineImageControl
    {
        public ExineLabel nameLabel, typeLabel, goldLabel, gpLabel, stockLabel, StockLabel, countLabel;
        public GameShopItem Item;
        public UserItem ShowItem;
        Rectangle ItemDisplayArea;
        public MirButton BuyItem, PreviewItem;
        public ExineImageControl ViewerBackground;
        public byte Quantity = 1;
        public MirButton quantityUp, quantityDown;
        public ExineLabel quantity;
        ...
        public GameShopCell()
        {
            ...
            PreviewItem.Click += (o, e) =>
                {
                    ExineMainScene.Scene.GameShopDialog.Viewer.Dispose();
                    ExineMainScene.Scene.GameShopDialog.Viewer = new GameShopViewer
                    {
                        Parent = ExineMainScene.Scene.GameShopDialog,
                        Visible = true,
                        Location = this.Location.X < 350 ? new Point(416, 115) : new Point(151, 115),
                    };
                    ExineMainScene.Scene.GameShopDialog.Viewer.ViewerItem = Item;
                    ExineMainScene.Scene.GameShopDialog.Viewer.UpdateViewer();
                };
        ...
        public void BuyProduct()
        {
            ...
            int pType = -1;
            if (ExineMainScene.Scene.GameShopDialog.PaymentTypeCredit.Checked && Item.CanBuyCredit)
            {
                pType = 0;
            }
            else if (ExineMainScene.Scene.GameShopDialog.PaymentTypeGold.Checked && Item.CanBuyGold)
            {
                pType = 1;
            }
            if (pType == -1)
            {
                ExineMainScene.Scene.ExChatDialog.ReceiveChat("You MUST select a payment type!", ChatType.System);
                return;
            }
            switch (pType)
            {
                case 0: //  Credit
                    if (Item.CreditPrice * Quantity <= ExineMainScene.Credit)
                    {
                        ...
                        ExineMainScene.Scene.ExChatDialog.ReceiveChat("You can't afford the selected item.", ChatType.System);
                    break;
                case 1: //  Gold
                    if (Item.GoldPrice * Quantity <= ExineMainScene.Gold)
                    {
        ...
        public override void OnMouseMove(MouseEventArgs e)
        {
            ...
            if (ExineMainScene.HoverItem != null && (Item.Info.Index != ExineMainScene.HoverItem.Info.Index))
            {
                ExineMainScene.Scene.DisposeItemLabel();
                ExineMainScene.HoverItem = null;
                ShowItem = null;
        ...
        public void UpdateText()
        {
            ...
            nameLabel.ForeColour = ExineMainScene.Scene.GradeNameColor(Item.Info.Grade);
            if (Item.CanBuyGold)
    ...
    public sealed class GameShopViewer : ExineImageControl
    {

        public ExineAnimatedControl PreviewImage, WeaponImage, WeaponImage2, MountImage;
        ...
        public GameShopViewer()
        {
            ...
            WeaponImage = new ExineAnimatedControl
            {
                Animated = false,
            ...
            WeaponImage2 = new ExineAnimatedControl
            {
                Animated = false,
                Location = new Point(105, 160),
                AnimationCount = 6,
            ...
            MountImage = new ExineAnimatedControl
            {
                Animated = false,
                Location = new Point(105, 160),
                AnimationCount = 8,
            ...
            PreviewImage = new ExineAnimatedControl
            {
                Animated = false,
                Location = new Point(105, 160),
                AnimationCount = 6,
        ...
        private void DrawMount()
        {
            ...
            if (ExineMainScene.User.Equipment[(int)EquipmentSlot.Armour] != null)
                PreviewImage.Library = Libraries.CArmours[ExineMainScene.User.Equipment[(int)EquipmentSlot.Armour].Info.Shape];
            else
                PreviewImage.Library = Libraries.CArmours[0];

            if (ExineMainScene.User.Gender == ExineGender.Male)
