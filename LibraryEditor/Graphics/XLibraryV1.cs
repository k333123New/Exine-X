...
namespace LibraryEditor
{
    ...
    /// Uses DXT1 Images
    /// </summary>
    public sealed class XLibraryV1
    {
        public const int LibVersion = 1;
        public static bool Load = true;
        public string FileName;
        
        public List<ExineImage> Images = new List<ExineImage>();
        ...
        public XLibraryV1(string filename)
        {
            FileName = filename;
            Initialize();
            Close();
        ...
        private void CheckImage(int index)
        {
            ...
            if (Images[index] == null)
            {
                ...
                Images[index] = new ExineImage(_reader);
            }

            if (!Load) return;

            ExineImage mi = Images[index];
            if (!mi.TextureValid)
            {
        ...
        public void ToMLibrary()
        {
            ...
            XLibraryV2 library = new XLibraryV2(fileName) { Images = new List<XLibraryV2.ExineImage>(), IndexList = new List<int>(), Count = Images.Count };
            //library.Save();
            ...
            try
            {
                ...
                {
                    ExineImage image = Images[i];
                    if (image.HasMask)
                        library.Images[i] = new XLibraryV2.ExineImage(image.Image, image.MaskImage) { X = image.X, Y = image.Y, ShadowX = image.ShadowX, ShadowY = image.ShadowY, Shadow = image.Shadow, MaskX = image.X, MaskY = image.Y };
                    else
                        library.Images[i] = new XLibraryV2.ExineImage(image.Image) { X = image.X, Y = image.Y, ShadowX = image.ShadowX, ShadowY = image.ShadowY, Shadow = image.Shadow };
                });
            }
        ...
        public ExineImage GetMImage(int index)
        {
        ...
        {
            ExineImage mImage = new ExineImage(image) { X = x, Y = y };
        ...
        public sealed class ExineImage
        {
            ...
            public ExineImage(BinaryReader reader)
            {
                //read layer 1
            ...
            public ExineImage(byte[] image, short Width, short Height)//only use this when converting from old to new type!
            {
                FBytes = image;
                this.Width = Width;
                this.Height = Height;
            ...
            public ExineImage(Bitmap image)
            {
                if (image == null)
                {
                    FBytes = new byte[0];
            ...
            public ExineImage(Bitmap image, Bitmap Maskimage)
            {
                if (image == null)
                {
                    FBytes = new byte[0];
