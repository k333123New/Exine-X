            ...
            for (int i = 0; i < CheckList.Count; i++)
            {
                ...
                switch (check.Type)
                {
                    ...
                    case CheckType.CheckGender:
                        ...
                        if (!ExineGender.TryParse(param[0], false, out gender))
                        {
                            failed = true;
                            break;
                        }
                    ...
                    case CheckType.CheckClass:
                        ...
                        if (!ExineClass.TryParse(param[0], true, out mirClass))
                        {
                            failed = true;
                            break;
                        }
            ...
            for (var i = 0; i < acts.Count; i++)
            {
                ...
                switch (act.Type)
                {
                    ...
                    case ActionType.ChangeGender:
                        {
                            switch (player.Info.Gender)
                            {
                                case ExineGender.Male:
                                    player.Info.Gender = ExineGender.Female;
                                    break;
                                case ExineGender.Female:
                                    player.Info.Gender = ExineGender.Male;
                                    break;
                            }
                        }
                        break;
                    ...
                    case ActionType.ChangeClass:
                        {
                            if (!Enum.TryParse(param[0], true, out ExineClass mirClass)) return;

                            switch (mirClass)
                            {
                                case ExineClass.Warrior:
                                    player.Info.Class = ExineClass.Warrior;
                                    break;
                                case ExineClass.Taoist:
                                    player.Info.Class = ExineClass.Taoist;
                                    break;
                                case ExineClass.Wizard:
                                    player.Info.Class = ExineClass.Wizard;
                                    break;
                                case ExineClass.Assassin:
                                    player.Info.Class = ExineClass.Assassin;
                                    break;
                                case ExineClass.Archer:
                                    player.Info.Class = ExineClass.Archer;
                                    break;
                            }
                        }
                        break;
