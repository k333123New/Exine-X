...
namespace Server.ExineDatabase
{
    public class RecipeInfo
    {
        ...
        public List<ExineClass> RequiredClass = new List<ExineClass>();
        public ExineGender? RequiredGender = null;
                            ...
                            try
                            {
                                switch (data[0].ToLower())
                                {
                                    case "level":
                                        ...
                                        break;
                                    case "class":
                                        if (Enum.TryParse<ExineClass>(data[1], true, out ExineClass cls))
                                        {
                                            RequiredClass.Add(cls);
                                        }
                                        else
                                        {
                                            RequiredClass.Add((ExineClass)byte.Parse(data[1]));
                                        }
                                        break;
                                    case "gender":
                                        if (Enum.TryParse<ExineGender>(data[1], true, out ExineGender gender))
                                        {
                                            RequiredGender = gender;
                                        }
                                        else
                                        {
                                            RequiredGender = (ExineGender)byte.Parse(data[1]);
                                        }
                                        break;
                                    case "flag":
