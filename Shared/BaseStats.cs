...
{
    public ExineClass Job;
    public List<BaseStat> Stats = new List<BaseStat>();

    public Stats Caps = new Stats();

    public BaseStats(ExineClass job)
    {
        Job = job;
...
public class BaseStat
{
    ...
    public int Calculate(ExineClass job, int level)
    {
        ...
        if (FormulaType == StatFormula.Health)
        {
            return job switch
            {
                ExineClass.Warrior => (int)Math.Min(Max > 0 ? Max : int.MaxValue, Base + (level / Gain + GainRate + level / 20F) * level),
        ...
        else if (FormulaType == StatFormula.Mana)
        {
            return job switch
            {
                ExineClass.Wizard => (int)Math.Min(Max > 0 ? Max : int.MaxValue, Base + ((level / Gain + 2F) * 2.2F * level) + (level * GainRate)),
                ExineClass.Taoist => (int)Math.Min(Max > 0 ? Max : int.MaxValue, (Base + level / Gain * 2.2F * level) + (level * GainRate)),
