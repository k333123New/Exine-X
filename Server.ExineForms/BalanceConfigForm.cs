...
namespace Server
{
    public partial class BalanceConfigForm : Form
    {
        ...
        {
            InitializeComponent();
            ClassComboBox.Items.AddRange(Enum.GetValues(typeof(ExineClass)).Cast<object>().ToArray());
        ...
        private void UpdateClassStatExample(Stat type)
        {
            ...
            var str = string.Format("{0} - {1}\r\r", ((ExineClass)SelectedClassID), type);
            ...
            if (stat != null)
            {
                ...
                {
                    str += string.Format("Level {0}\t: {1}\n", level, Math.Min(cap, stat.Calculate((ExineClass)SelectedClassID, level)));
                }
