        ...
        public static void Start(Envir envirToUpdate)
        {
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                if (lines[i].StartsWith("[")) // Read map info
                {
                    ...
                    ExineDatabase.MapInfo newMapInfo = new ExineDatabase.MapInfo { Index = ++EditEnvir.MapIndex };
            ...
            for (int j = 0; j < EditEnvir.MapInfoList.Count; j++)
            {
                for (int k = 0; k < lines.Length; k++)
                {
                    try
                    {
                        ...
                        {
                            ExineDatabase.MovementInfo newMovement = new ExineDatabase.MovementInfo();
